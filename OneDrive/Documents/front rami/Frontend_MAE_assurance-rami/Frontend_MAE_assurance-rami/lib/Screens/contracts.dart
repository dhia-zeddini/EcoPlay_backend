...
import '../Services/ContractService.dart';
import '../Services/payment_service.dart';
import 'package:intl/intl.dart';

import 'claim.dart'; // Import the intl package for DateFormat
...
class _ContractsScreenState extends State<ContractsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetchContractsForUser() async {
    final contractService = ContractService(baseUrl: 'http://192.168.65.31:9001');
    final contracts = await contractService.fetchContractsForUser();
    setState(() {
      _contracts = contracts;
      _filteredContracts = contracts;
    });
    _checkAndMarkExpiredContracts(); // Check and mark expired contracts
  }
  ...
  Widget build(BuildContext context) {
    ...
    return FancyDrawerWrapper(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Contracts Screen'),
          leading: IconButton(
  ...
  void _checkAndMarkExpiredContracts() {
  ...
  Widget _buildContractsList() {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        String formattedStartDate = DateFormat('yyyy-MM-dd HH:mm:ss').format(contract.startDate);
        String formattedEndDate = DateFormat('yyyy-MM-dd HH:mm:ss').format(contract.endDate);
          ...
          child: ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                ...
                SizedBox(height: 5),
                Text('Start Date: $formattedStartDate'),  // Display formatted start date
                SizedBox(height: 5),
                Text('End Date: $formattedEndDate'),  // Display formatted end date
              ],
            ),
